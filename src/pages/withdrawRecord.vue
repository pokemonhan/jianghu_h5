    ...
    <div class="withdrawRecord">
        ...
        <div class="contentView">
            ...
            <div class="recordBar animated faster" :class="{fadeInLeft:index%2===0,lightSpeedIn:index%2===1}" v-for="(item,index) in recordList" @click="showDetail(item)">
                <div class="leftSide">
                    <div class="itemImg">
                        <img class="iconImg" :src="item.account_type===1?iconList['支付宝转账']:iconList['银联支付']"/>
                    </div>
                    <div class="amountBar">
                        <div class="amount" v-text="'金额：'+item.amount"></div>
                        <div class="rechargeType" v-text="item.account_type===1?'支付宝提款':'银行卡提款'"></div>
                    </div>
                </div>
                <div class="rightSide">
                    <div class="timeBar">
                        <div class="status" :class="{red:item.status===2 || item.status===6,green:item.status===3 || item.status===4 || item.status===5,yellow:item.status===0 || item.status===1}" v-text="(item.status===0　|| item.status===1)?'审核中':(item.status===3 || item.status===4 || item.status===5)?'提款成功':'提款失败'"></div>
    ...
    export default {
        ...
        methods:{
            ...
            chooseTime(index){
                ...
                data={type:3,created_at:this.time};
                if(index===0)data={type:3};
                all.tool.send("record",data,res=>{
                    this.recordList=res.data.data;
                    if(res.data.total===0)all.tool.editTipShow("暂无提款记录")
                })
            },
            showDetail(item){
                all.tool.setStore("withdrawDetail",item);
                this.open("withdrawDetail")
            }
        },
        created() {
            all.tool.send("record",{type:3},res=>{
                this.recordList=res.data.data;
                if(res.data.total===0)all.tool.editTipShow("暂无提款记录")
            })
        }

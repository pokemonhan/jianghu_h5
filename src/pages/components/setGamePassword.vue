<template>
    <div class="setGamePassword">
        <div class="setBox animated fadeInDown faster">
            ...
            <span class="setTitle">设置密码</span>
            <div class="setBar">
                <div class="inputItem">
                    <div class="itemIcon">
                        <img class="icon" src="../../assets/login/icon_Lock.png"/>
                    </div>
                    <input :type="passwordState" class="inputEdit" v-model="password" placeholder="请设置您的游戏密码" @blur="checkPassword" @keyup="limitPassword" maxlength="40">
    ...
    export default {
        ...
        methods:{
            ...
            toDoClose(){
                ...
                setTimeout(()=>{all.store.commit("setGamePassword",{isShow:false,registerUrl:null,item:null})},150)
            },
            toDoCommit(){
                if(this.checkPassword()){
                    all.tool.send("setPassword"+this.$store.state.setGamePassword.registerUrl,{password:this.password},res=>{
                        all.tool.send("openGame"+this.$store.state.setGamePassword.item.url,null,result=>{
                            all.tool.setStore("gameFrameTitle",this.$store.state.setGamePassword.item.name);
                            all.tool.setStore("gameFrameUrl",result.data.url);
                            this.toDoClose();
